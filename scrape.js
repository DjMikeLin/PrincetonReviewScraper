...
(async () => {
    ...
    for(let i = 2; i < process.argv.length; i++){
        ...
        let gpaBreakdown = await page.$$eval('#admissions > .row > .col-sm-9 > .row > .col-sm-4 > .row.graph-row-container.gpa-chart-data',
            nodes =>
                nodes.map(element => {
        ...
        let deadlines = await page.$$eval('#admissions > .row > .col-sm-9 > p',
          nodes =>
            nodes.map(element => {
        ...
        let selectivityRating = await page.$$eval('#admissions > .row > .col-sm-9 > .row > .col-sm-6 > .number-callout',
          nodes =>
            nodes.map(element => {
        ...
        let facultyStudentInfo2 = await page.$$eval('#academics > .row > .col-sm-9 > .row.graph-row-container',
          nodes =>
            nodes.map(element => {
        ...
        let graduationRates = await page.$$eval('#academics > .row > .col-sm-9 > .row > .col-sm-4',
          nodes =>
            nodes.map(element => {
        ...
        let majors = await page.$$eval('#academics > .row > .col-sm-9 > div > .list-unstyled',
          nodes =>
            nodes.map(element => {
                return element.innerText;
            })    
        ...
        let prominentAlumni = await page.$$eval('#academics > .row > .col-sm-9 > .row',
          nodes =>
            nodes.map(element => {
                if(element.querySelector('.col-xs-7.col-sm-7') !== null && element.querySelector('.col-xs-5.col-sm-5.text-right > .bold') !== null)
        ...
        let financialAidStats = await page.$$eval('#tuition > .row > .col-sm-9 > .row',
          nodes =>
            nodes.map(element => {
        ...
        let raceDemographics = await page.$$eval('#studentbody > .row > .col-sm-9 > div > .row.graph-row-container',
          nodes =>
            nodes.map(element => {
                if(element.querySelector('.col-xs-2.col-sm-2.bold') !== null && element.querySelector('.col-xs-5.col-sm-3') !== null)
                    return element.querySelector('.col-xs-5.col-sm-3').innerText.trim() + ' : ' + element.querySelector('.col-xs-2.col-sm-2.bold').innerText.trim();
        ...
        let otherDemographics = await page.$$eval('#studentbody > .row > .col-sm-9 > .row.graph-row-container > .col-xs-12 > .row',
          nodes =>
            nodes.map(element => {
        ...
        let percentInClubAndSports = await page.$$eval('#campuslife > .row > .col-sm-9 > .row.graph-row-container > .col-xs-12 > .row > .col-xs-6',
          nodes =>
            nodes.map(element => {
